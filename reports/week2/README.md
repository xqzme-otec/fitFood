...
> The IDs below are the **stable IDs** defined in
> [`user-stories.md`](user-stories.md#note-on-user-story-id-numbering). The customer
> meeting [transcript](customer-meeting-transcript.md) numbers stories differently (by
> live discussion order) — see that document's mapping table to reconcile the two.
